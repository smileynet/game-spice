...
- `(see game-difficulty-design/difficulty-modes.md for Traditional Difficulty Modes, Assist Mode Design (Celeste Model), Accessibility Planning)`
- `(see game-difficulty-design/design-depth.md for Difficulty Curve Guidelines, Recovery Design Guidelines, Boss Design Principles)`
- `(see game-difficulty-design/anti-patterns.md for Fake Difficulty Taxonomy, developer blind spot, number inflation, grind gates)`

## See Also

- **game-design-frameworks** — Flow theory context, MDA aesthetics for targeting "Challenge" `(see game-design-frameworks)`
- **game-playtesting** — Testing for difficulty specifically, observation methodology `(see game-playtesting)`
- **game-scoping** — Difficulty modes as a scope decision `(see game-scoping → MLP Scoping Process)`
- **game-content-planning** — Difficulty pacing through content `(see game-content-planning)`
- **game-antipatterns** — Premature Content and Core Loop Neglect apply to difficulty tuning `(see game-antipatterns)`
- **game-economy-design** — Resource scarcity as difficulty lever, economy progression curves `(see game-economy-design)`
...
- **game-mechanics-palette** — Mechanic choices that determine challenge type `(see game-mechanics-palette)`
