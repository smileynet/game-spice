...
`(see game-architecture-audit → Game Loop Patterns)`
...
`(see game-architecture-audit/detailed-audits.md for State Management)`
...
`(see game-architecture-audit/detailed-audits.md for Input Handling)`
...
`(see game-architecture-audit/detailed-audits.md for Performance Smells)`
...
`(see game-architecture-audit/detailed-audits.md for Entity Architecture)`
...
- **game-architecture-audit** — Audit existing code against these patterns `(see game-architecture-audit → Architecture Health Checklist)`
- **game-antipatterns** — Design mistakes that lead to bad architecture `(see game-antipatterns)`
- **game-scoping** — Scope determines acceptable shortcuts `(see game-scoping → MLP Scoping Process)`
