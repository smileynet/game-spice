...
- `(see mechanics-palette/palette.md for all 10 mechanic categories with full tables and design notes)`
- `(see mechanics-palette/mashups.md for mashup pattern deep dive: Loop Nesting, Verb+Context Shift, Opposing Forces, Scale Shift, System Merger)`
- `(see mechanics-palette/exploration-prompts.md for guided 6-step discovery process for brainstorming sessions)`

## See Also

- **design-frameworks** — MDA for evaluating mechanic combinations, core loop design for integration testing `(see design-frameworks → MDA Framework)`
- **scoping** — Turn mechanic selections into an MLP scope `(see scoping → MLP Scoping Process)`
- **antipatterns** — Kitchen Sink warning: too many mechanics without integration `(see antipatterns/design-antipatterns.md → Kitchen Sink Design)`
- **economy-design** — Resource mechanics need economy planning `(see economy-design)`
- **difficulty-design** — Challenge type selection follows from mechanic choices `(see difficulty-design → Challenge Types)`
